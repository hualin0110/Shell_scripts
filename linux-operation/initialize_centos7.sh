...
#only for hwcloud
ntp_server="ntp.myhuaweicloud.com"
...
service_setting(){
	get_time "设置开机启动的服务"
	apps_disable='acpid anacron atd auditd autofs avahi-daemon bluetooth cpuspeed cups firstboot gpm haldaemon hidd hplip ip6tables irqbalance  isdn kudzu lvm2-monitor mcstrans mdmonitor messagebus microcode nfslock pcscd portmap readahead_early readahead_later restorecond rpcgssd  rpcidmapd postfix setroubleshoot smartd xinetd iptables nscd '
	apps_enable='crond netfs network sshd syslog sysstat xfs yum-updatesd'
	for i in $apps_disable; do
		systemctl disable $i
		# chkconfig $i off
	done
...
clean_postfix(){
	##清理postfix里面的垃圾信息
	tee /usr/local/bin/spool_clean.sh <<-'EOF'
		find /var/spool/clientmqueue/ -type f -mtime +30|xargs rm-f
EOF
	echo '*/30 * * * * /bin/sh /usr/local/bin/spool_clean.sh >/dev/null 2>&1' >> /var/spool/cron/root
}
...
ntp_install(){
	...
	#第二种
	echo "* 1 * * * /usr/sbin/ntpdate -u ntp.myhuaweicloud.com 2>&1 >/dev/null" >> /var/spool/cron/root
