...
#        f.write(subprocess.getoutput("/usr/bin/fdfs_monitor /etc/fdfs/storage.conf"))
    total_space = int(subprocess.getoutput('grep "disk total space" %s |awk \'{sum += $5} END {print sum}\'' % Disk_detail_tmp))
    free_space = int(subprocess.getoutput('grep "disk free space" %s |awk \'{sum += $5} END {print sum}\'' % Disk_detail_tmp))
...
def check_time():
    nowTime = time.time()
    lastTime = int(subprocess.getoutput('cat %s' %Last_time_tmp))
    ##以小时作为时间单位。
